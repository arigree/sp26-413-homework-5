...
function renderKPIs(rows) {
  ...
  const avgCVR =
    rows.reduce((sum, r) => sum + r.conversionRate, 0) / rows.length;
  ...
  document.getElementById("avg-bounce-rate").textContent =
    avgBounceRate.toFixed(1) + "%";
  document.getElementById("avg-conversion-rate").textContent =
    avgCVR.toFixed(1) + "%";
  document.getElementById("total-pageviews").textContent =
    totalPageviews.toLocaleString();
  document.getElementById("total-new-visitors").textContent =
  ...
  barChartInstance = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (ctx) => ctx.raw.toLocalString(),
          },
        },
      },
      scales: {
...
function setupToggle() {
  ...
  btnRevenue.addEventListener("click", () => {
    ...
    drawBarChart(allData, currentMetric);
    title.textContent = "Revnue by Traffic Source $";
...
function setupFilter() {
  ...
  filter.addEventListener("change", () => {
    ...
    if (value === "All") {
      ...
      const filtered = allData.filter((r) => r.source === value);
      buildTable(filtered);
    }
  });
}
